...
function HistorySidebar({
  ...
  return (
    <SidebarRight isOpen={isOpen} onClose={onClose}>
      ...
      <div className="my-8 flex h-5/6 flex-col gap-4 overflow-y-auto px-6">
        {history.map((history: any, index: number) => (
          <div
            key={index}
            className="flex divide-x-2 divide-border rounded-xl border-2 border-border"
          >
            <div
              className="flex cursor-pointer flex-col gap-2 rounded-l-lg p-4 hover:bg-secondary"
              onClick={() => {}}
            >
              <Markdown className="line-clamp-2 text-sm font-medium text-body">
                {history.summary}
              </Markdown>
