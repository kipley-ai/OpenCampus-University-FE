...
import ModalLoginTwitter from "@/components/modal-login-twitter-iframe";
...
export default function DataSource({
  ...
  const twSuccess = searchParams.get("ts");
  const nextStep = searchParams.get("nextStep");

  if (twSuccess && twSuccess === "1") {
    return redirect(
      `${process.env.NEXT_PUBLIC_KIPPRO_URL}/knowledge/create?nextStep=mint_nft`,
    );
  }
  ...
  if (
    ...
    setStep("mint_nft");
    router.push("/knowledge/create/iframe");
  }
