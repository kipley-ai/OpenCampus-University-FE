      ...
      <div className="mb-16 flex flex-col gap-8 md:mb-24">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-5">
          {apps.map((app) => (
            <button
              key={app.name}
              className={`flex h-36 flex-col items-center justify-center gap-2 rounded-2xl border-2 bg-container px-8 py-6 text-lg font-medium xs:text-xl ${app.step === "" ? "disabled text-secondary-text" : "text-primary"} enabled:hover:bg-secondary ${chosenApp === app.step ? "border-primary" : "border-border"}`}
