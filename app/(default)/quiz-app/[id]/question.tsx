            ...
            {questions &&
              ["a", "b", "c", "d"].map((choices: any, index: any) => {
                ...
                return (
                  <label
                    key={index}
                    className={`relative flex w-full cursor-pointer flex-row rounded-lg border border-gray-300 px-4 py-3 text-left ${selected_answer === choice ? "bg-[#ECECFF] font-medium" : ""} ${step === "question" ? "hover:bg-secondary" : ""}`}
                  >
                    <input
                      type="radio"
                      name="quiz"
