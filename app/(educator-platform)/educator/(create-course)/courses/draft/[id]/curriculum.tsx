...
const Lesson = ({ number, lesson }: LessonProps) => {
...
const Section = ({ number, section }: SectionProps) => {
  const [addItemStep, setAddItemStep] = useState<string>("DEFAULT");

  const { id } = useParams();
  const lessons = useLessonList({
    data_status: 1,
    published: 0,
    created_by: "799a5457-a0ca-466b-8f78-545308aaa72e",
    course_uuid: id,
  ...
  return (
    <div className="mt-8 rounded-lg border border-border bg-container p-6 text-heading">
      <h2>
        <span className="font-bold text-heading">Section {number}: </span>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="12"
          height="14"
      ...
      {lessons.isPending ? (
        ...
        <>
          {lessons.data.data.data.lessons.map((lesson: any, index: number) => (
            <>
              <Lesson key={index} number={index + 1} lesson={lesson} />
            </>
          ))}
        </>
      )}
      ...
      {(() => {
        switch (addItemStep) {
          case "DEFAULT":
            return (
              <button
                onClick={() => setAddItemStep("CHOOSE_TYPE")}
                className="mb-8 mt-4 flex w-fit items-center gap-2 rounded-lg border border-border bg-sidebar px-4 py-2 text-primary hover:bg-secondary"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="14"
          ...
          case "CHOOSE_TYPE":
            return (
              <div className="mt-4 flex items-start gap-8">
                <button
                  onClick={() => setAddItemStep("DEFAULT")}
                  className="ml-1 mt-2 hover:text-primary"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="12"
                  ...
                  >
                    <path
                      fill="currentColor"
                      d="M11.833 1.832 10.658.657 5.999 5.315 1.341.657.166 1.832 4.824 6.49.166 11.148l1.175 1.175 4.658-4.658 4.659 4.658 1.175-1.175L7.174 6.49l4.659-4.658Z"
                    />
                  </svg>
                </button>
                <div className="flex items-center gap-4 border border-dashed border-[#6A6F73] bg-sidebar px-6 py-3 font-medium text-primary">
                  <button
                    onClick={() => setAddItemStep("LESSON")}
                    className="flex items-center justify-between gap-2 enabled:hover:text-secondary disabled:text-body"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
            ...
            return (
              <div className="rounded-lg border border-border bg-sidebar p-4">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="text"
                    className="grow rounded-lg border border-border px-3 py-2 text-sm"
  ...
  return (
    <div className="text-sm">
      ...
      {sections.isPending ? (
        ...
        <>
          {sections.data.data.data.courses.map(
