...
export default function Step1({ selectedButton, setSelectedButton }: { selectedButton: string, setSelectedButton: Function }) {
    ...
    return (
        <div className="mx-64 grid grid-cols-2 gap-4 text-white font-bold mt-10">
            <button className={`flex flex-col px-20 py-10 items-center border-2 ${selectedButton == 'twitter' ? 'border-aqua-700' : 'border-[#50575F]'} rounded-3xl`} onClick={() => setSelectedButton('twitter')}>
                <Image width={48} height={48} src={XIcon} alt="X Icon" />
                <h3 className="pt-6">Connect Twitter</h3>
            </button>
            <button className={`flex flex-col px-20 py-10 items-center border-2 ${selectedButton == 'notion' ? 'border-aqua-700' : 'border-[#50575F]'} rounded-3xl`} onClick={() => setSelectedButton('notion')}>
                <Image width={48} height={48} src={NotionIcon} alt="Notion Icon" />
                <h3 className="pt-6">Connect Notion</h3>
            </button>
            <button className={`flex flex-col px-20 py-10 items-center border-2 ${selectedButton == 'local' ? 'border-aqua-700' : 'border-[#50575F]'} rounded-3xl`} onClick={() => setSelectedButton('local')}>
