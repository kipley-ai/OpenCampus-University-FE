  ...
  return (
    <div className="flex flex-row space-x-4 px-4 py-3">
      ...
      <div className="flex w-full flex-col gap-2 pt-2 text-sm">
        <p>{message.sender === "user" ? "You" : chatbotData?.data.data.name}</p>
...
export const ShareChatroomModal = ({
  isOpen,
  setIsOpen,
  messageHistory,
  chatbotData,
  ...
  const sharedIdAPI = useGetSharedChatId({
    chatbot_id: chatbotData?.room_id,
  });
    ...
    if (sharedIdAPI.isSuccess && sharedIdAPI.data.data.data) {
      ...
      if (!sharedIdAPI.isRefetching && copyClipboard) {
        ...
        navigator.clipboard.writeText(
          `${BASE_URL}/share/chatroom/${sharedIdAPI.data.data.data.share_id}`,
        );
        setTimeout(() => {
