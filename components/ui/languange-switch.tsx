...
const LanguangeSwitch = () => {
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      {isDropdownOpen && (
        <div className="absolute right-0 z-50 mt-2 w-48 rounded-md bg-container py-2 text-[9px] shadow-xl xs:w-56 xs:text-xs sm:text-[0.8rem]">
          <div className="mx-4 border-t border-border"></div>
          {languages.map((language) => (
            <button
              key={language}
              className="mx-2 block flex w-[95%] items-center rounded-md px-2 py-2 hover:bg-secondary"
              onClick={() => handleLanguageChange(language)}
            >
              {lang === language && (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2">
                  <path d="M9.5 17L4.5 12" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  <path d="M19.5 7L9.5 17" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              )}
              <span className={lang === language ? "font-semibold" : ""}>{language}</span>
            </button>
          ))}
        </div>
      )}
